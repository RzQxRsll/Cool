...
local Config = {
    ...
    MOBILE_Mode = TouchEnabled,
    MOBILE_MenuButton = {x = 10, y = 10, size = 50},
    MOBILE_FloatingButtons = true,
    MOBILE_ButtonOpacity = 0.8,
    MOBILE_TapToSelect = true,
    MOBILE_SwipeSensitivity = 30,
    MOBILE_LongPressDuration = 0.5
}
...
local MenuItems = {
    ...
    -- Mobile toggle added to MISC tab
    {tab = 6, name = "MOBILE", type = "label"},
    {tab = 6, name = "Mobile Mode", key = "MOBILE_Mode", type = "toggle"},
    {tab = 6, name = "Floating Button", key = "MOBILE_FloatingButtons", type = "toggle"}
}
...
-- Create floating menu button for mobile
function Mobile.CreateFloatingButton()
    if not Config.MOBILE_FloatingButtons or not Config.MOBILE_Mode then return end
    
    local btn = {
        bg = Drawing.new("Circle"),
        icon = Drawing.new("Text"),
        border = Drawing.new("Circle")
    }
    
    btn.bg.Filled = true
    btn.bg.Color = Color3.fromRGB(30, 30, 40)
    btn.bg.Transparency = 0.8
    btn.bg.NumSides = 30
    btn.bg.Visible = false
    
    btn.border.Filled = false
    btn.border.Color = Color3.fromRGB(220, 70, 70)
    btn.border.Thickness = 2
    btn.border.NumSides = 30
    btn.border.Visible = false
    
    btn.icon.Size = 20
    btn.icon.Font = Drawing.Fonts.UI
    btn.icon.Center = true
    btn.icon.Color = Color3.fromRGB(235, 235, 240)
    btn.icon.Text = "☰"
    btn.icon.Visible = false
    
    Mobile.floatingButton = btn
end

function Mobile.UpdateFloatingButton(cam)
    if not Config.MOBILE_FloatingButtons or not Config.MOBILE_Mode then 
        if Mobile.floatingButton then
            if Mobile.floatingButton.bg then Mobile.floatingButton.bg.Visible = false end
            if Mobile.floatingButton.border then Mobile.floatingButton.border.Visible = false end
            if Mobile.floatingButton.icon then Mobile.floatingButton.icon.Visible = false end
        end
        return 
    end
    
    if not Mobile.floatingButton then
        Mobile.CreateFloatingButton()
    end
    
    local pos = Vector2.new(10, 10)
    local size = 50
    
    Mobile.floatingButton.bg.Position = pos
    Mobile.floatingButton.bg.Radius = size/2
    Mobile.floatingButton.bg.Visible = not Config.MENU_Open
    
    Mobile.floatingButton.border.Position = pos
    Mobile.floatingButton.border.Radius = size/2
    Mobile.floatingButton.border.Visible = not Config.MENU_Open
    
    Mobile.floatingButton.icon.Position = Vector2.new(pos.X, pos.Y - 2)
    Mobile.floatingButton.icon.Visible = not Config.MENU_Open
end

function Mobile.HandleTouchBegan(touch)
    local pos = touch.Position
    local now = tick()
    
    Mobile.touches[touch.UserId] = touch
    Mobile.touchStartPos[touch.UserId] = pos
    Mobile.touchStartTime[touch.UserId] = now
    
    -- Check floating button first
    if Config.MOBILE_FloatingButtons and Config.MOBILE_Mode and Mobile.floatingButton and Mobile.floatingButton.bg.Visible then
        local btnPos = Vector2.new(10, 10)
        local btnSize = 25
        local dist = (pos - btnPos).Magnitude
        if dist <= btnSize then
            Config.MENU_Open = not Config.MENU_Open
            return
        end
    end
    
    -- Check GUI interaction when menu is open
    if Config.MENU_Open and GUI.Position then
        ...
        -- Check drag bar
        if pos.X >= x + sidebarW and pos.X <= x + menuW and
           pos.Y >= y and pos.Y <= y + dragBarH then
            Mobile.isDragging = true
            Mobile.dragStartPos = pos
            GUI.PositionOffset = pos - GUI.Position
            return
        end
        
        -- Check tabs
        for i, tabPos in pairs(GUI.TabPositions) do
            if pos.X >= tabPos.x and pos.X <= tabPos.x + tabPos.w and
               pos.Y >= tabPos.y and pos.Y <= tabPos.y + tabPos.h then
                Config.MENU_Tab = i
                SelectedItem = 1
                GUI.ScrollOffset = 0
        ...
        if GUI.ScrollbarBounds and GUI.MaxScroll > 0 then
            ...
            if pos.X >= sb.x and pos.X <= sb.x + sb.w + 10 and
               pos.Y >= sb.y and pos.Y <= sb.y + sb.h then
                Mobile.scrollStartY = pos.Y
                Mobile.scrollStartOffset = GUI.ScrollOffset
                return
            end
        end
        
        -- Check items
        for i, itemPos in pairs(GUI.ItemPositions) do
            if pos.X >= itemPos.x and pos.X <= itemPos.x + itemPos.w and
               pos.Y >= itemPos.y and pos.Y <= itemPos.y + itemPos.h then
                local tabItems = {}
                ...
                local menuItem = tabItems[itemPos.idx]
                if menuItem then
                    SelectedItem = itemPos.idx
                    ...
                    elseif menuItem.type == "slider" then
                        Mobile.activeFinger = touch.UserId
                        local sliderX = itemPos.x + 10
                        local sliderW = itemPos.w - 60
                        local pct = math.clamp((pos.X - sliderX) / sliderW, 0, 1)
        ...
        if Mobile.longPressTimer then
            Mobile.longPressTimer:Cancel()
        end
        Mobile.longPressTimer = task.delay(0.5, function()
            if Mobile.touches[touch.UserId] and not Config.MENU_Open then
                Config.MENU_Open = true
            end
        end)
    end
    
    -- Handle aimbot on touch
    if Config.AIM_Enabled and not Config.AIM_UseRMB then
        State.AimHolding = true
    end
end

function Mobile.HandleTouchMoved(touch)
    local pos = touch.Position
    local userId = touch.UserId
    
    if Config.MENU_Open then
        if Mobile.isDragging and GUI.Position then
            GUI.Position = pos - GUI.PositionOffset
            local cam = workspace.CurrentCamera
            if cam then
        ...
        elseif Mobile.scrollStartY then
            local deltaY = pos.Y - Mobile.scrollStartY
            local scrollDelta = deltaY * (GUI.MaxScroll / (GUI.ScrollbarBounds.h or 1)) * 2
            GUI.ScrollOffset = math.clamp(Mobile.scrollStartOffset + scrollDelta, 0, GUI.MaxScroll)
        elseif Mobile.activeFinger == userId then
            -- Update slider value
            for i, itemPos in pairs(GUI.ItemPositions) do
                if pos.X >= itemPos.x and pos.X <= itemPos.x + itemPos.w then
                    local tabItems = {}
                    for _, item in ipairs(MenuItems) do
                        if item.tab == Config.MENU_Tab then
                            table.insert(tabItems, item)
                    ...
                    local menuItem = tabItems[itemPos.idx]
                    if menuItem and menuItem.type == "slider" then
                        local sliderX = itemPos.x + 10
                        local sliderW = itemPos.w - 60
                        local pct = math.clamp((pos.X - sliderX) / sliderW, 0, 1)
                        local val = menuItem.min + pct * (menuItem.max - menuItem.min)
    ...
    Mobile.touches[userId] = touch
end

function Mobile.HandleTouchEnded(touch)
    local userId = touch.UserId
    local startPos = Mobile.touchStartPos[userId]
    local startTime = Mobile.touchStartTime[userId]
    local endPos = touch.Position
    
    if startPos and startTime then
        local delta = (endPos - startPos).Magnitude
        local duration = tick() - startTime
        
        -- Double tap detection
        if delta < 30 and duration < 0.3 then
            if tick() - Mobile.lastTapTime < 0.3 then
                Mobile.tapCount = Mobile.tapCount + 1
                if Mobile.tapCount >= 2 then
                    -- Double tap toggles aimbot
                ...
                Mobile.tapCount = 1
                Mobile.lastTapTime = tick()
            end
        end
    end
    
    -- Cleanup
    Mobile.touches[userId] = nil
    Mobile.touchStartPos[userId] = nil
    Mobile.touchStartTime[userId] = nil
    
    if userId == Mobile.activeFinger then
        Mobile.activeFinger = nil
    end
    
    if Mobile.longPressTimer then
        Mobile.longPressTimer:Cancel()
        Mobile.longPressTimer = nil
    end
    
    Mobile.isDragging = false
    Mobile.scrollStartY = nil
...
-- Modified GUI.Create function with mobile support
function GUI.Create()
    local menuW, menuH = 300, 420
    ...
    for i = 1, #Tabs do
        ...
        GUI.Drawings.Tabs[i].Label.Size = Config.MOBILE_Mode and 16 or 15
    ...
    for i = 1, 25 do
        ...
        item.Label.Size = Config.MOBILE_Mode and 16 or 15
        item.Label.Font = Drawing.Fonts.UI
        item.Label.Outline = true
        item.Label.Visible = false
        item.Value.Size = Config.MOBILE_Mode and 15 or 14
    ...
    GUI.Drawings.Hint.Size = Config.MOBILE_Mode and 14 or 13
    GUI.Drawings.Hint.Font = Drawing.Fonts.UI
    GUI.Drawings.Hint.Color = Colors.UI_TextDim
    GUI.Drawings.Hint.Outline = true
    GUI.Drawings.Hint.Text = Config.MOBILE_Mode and "Tap ☰ button | Long press menu" or "INS HOME TAB"
    GUI.Drawings.Hint.Visible = false
    
    GUI.Drawings.Role = Drawing.new("Text")
    GUI.Drawings.Role.Size = Config.MOBILE_Mode and 14 or 13
    ...
    GUI.Drawings.AutoGenHint.Size = Config.MOBILE_Mode and 18 or 16
...
function GUI.Render()
    ...
    local menuW, menuH = 300, 420
    local sidebarW = 60
    local dragBarH = Config.MOBILE_Mode and 30 or 22
    local footerH = Config.MOBILE_Mode and 40 or 30
    local scrollbarW = Config.MOBILE_Mode and 10 or 6
    local itemH = Config.MOBILE_Mode and 40 or 30
    local itemSpacing = Config.MOBILE_Mode and 6 or 4
    ...
    if Config.MOBILE_Mode then
        Mobile.UpdateFloatingButton(cam)
    end
end
...
local function HandleInput(input, gameProcessed)
    ...
    if Config.MOBILE_Mode and input.UserInputType == Enum.UserInputType.Touch then
        if input.UserInputState == Enum.UserInputState.Begin then
            Mobile.HandleTouchBegan(input)
        elseif input.UserInputState == Enum.UserInputState.Change then
            Mobile.HandleTouchMoved(input)
        elseif input.UserInputState == Enum.UserInputState.End then
            Mobile.HandleTouchEnded(input)
        end
        return
    end
    
    -- Original PC keyboard/mouse handling
...
Unload = function()
    ...
    -- Cleanup mobile button
    if Mobile.floatingButton then
        if Mobile.floatingButton.bg then Mobile.floatingButton.bg:Remove() end
        if Mobile.floatingButton.border then Mobile.floatingButton.border:Remove() end
        if Mobile.floatingButton.icon then Mobile.floatingButton.icon:Remove() end
        Mobile.floatingButton = nil
    end
